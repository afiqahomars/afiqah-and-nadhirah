...
# Custom CSS styling for a modern, tech-forward look
st.markdown("""
    <style>
    /* Main titles */
    .main-title {
...
if df is not None:
    ...
    # ==========================================
    st.sidebar.markdown("## 🍔 Delivery Ops AI")
    st.sidebar.markdown("---")
    ...
    # 2. Interactive Multi-select for Order Status (New Interactive Feature!)
    # Safe fallback if cancellation column is boolean or string
    ...
    # Apply global filtering based on sidebar inputs
    df_filtered = df[
    ...
    # ==========================================
    st.markdown('<p class="main-title">🚀 Food Delivery Performance & Operations</p>', unsafe_allow_html=True)
    st.markdown('<p class="sub-title">Interactive Live Intelligence Dashboard for Logistical Efficiency</p>', unsafe_allow_html=True)
    ...
    with kpi2:
        ...
        # Added dynamic color formatting via delta context if relevant
        st.metric(label="Total Cancellations", value=f"{total_canceled:,}", delta=f"{total_canceled/max(1, len(df_filtered))*100:.1f}% Rate", delta_color="inverse")
    with kpi3:
    ...
    with col1:
        ...
        fig_bar = px.bar(
            ...
            y='cancel_count',
            text='cancel_count',
            labels={'prep_time_bin': 'Preparation Time Window', 'cancel_count': 'Cancellations'},
    ...
    # --- CHART 2: PLOTLY LINE/AREA CHART (HOURLY ORDER TRENDS) ---
    with col2:
        ...
        # Interactive Plotly Area Chart for premium feel
        fig_line = px.area(
            orders_by_hour, 
            x='Order Hour (24h)', 
            y='Total Orders',
            labels={'Order Hour (24h)': 'Hour of Day (24h)', 'Total Orders': 'Orders Placed'},
    ...
    # ==========================================
    # CHARTS LAYOUT SECTION (ROW 2)
    # ==========================================
    # --- CHART 3: PLOTLY SCATTER PLOT WITH REGRESSION LINE ---
    ...
    if len(df_filtered) > 0:
        ...
        # Plotly Express builds quick trendlines effortlessly using OLS
        fig_scatter = px.scatter(
            df_sample, 
            x='Distance (km)', 
            y='Delivery Time (mins)',
            trendline="ols",
            trendline_color_override="#1E3A8A",
            color_discrete_sequence=['#2D3748'],
            opacity=0.5,
            labels={'Distance (km)': 'Distance Traveled (km)', 'Delivery Time (mins)': 'Total Delivery Transit (mins)'}
        )
        ...
        st.caption("💡 **Insight**: Strongly positive linear relationship verified. Hover over individual scatter points to trace absolute outlying logistical paths.")
    else:
        st.warning("No data available based on your current filter selection.")
